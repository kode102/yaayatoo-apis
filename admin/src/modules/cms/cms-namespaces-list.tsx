...
export default function CmsNamespacesListView() {
  ...
  async function toggleActive(row: CmsNamespaceDoc) {
    const token = await getIdToken();
    if (!token) return;
    setBusy(true);
    try {
  ...
  function openEdit(row: CmsNamespaceDoc) {
    setEditRow(row);
    setEditDrafts(
      ...
      <div className="overflow-x-auto rounded-xl border border-gray-200 bg-white shadow-sm">
        <table className="w-full min-w-[640px] text-left text-sm">
          <thead className="border-b border-gray-100 bg-gray-50/90 text-gray-500">
            <tr>
              <th className="px-3 py-2 font-medium">{t("cms.namespace.colKey")}</th>
